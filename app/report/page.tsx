...
export default function VOCReport() {
  ...
  // Section map
  const sectionMap = [
    { time: 20, id: 'sources-section' },
    { time: 37, id: 'executive-summary' },
    { time: 54, id: 'key-insights' },
    { time: 71, id: 'sentiment-chart' },
    { time: 88, id: 'mentions-topic' },
    { time: 105, id: 'trending-topics' },
    { time: 122, id: 'volume-over-time' },
    { time: 139, id: 'competitor-summary' },
    { time: 156, id: 'market-gaps' },
    { time: 173, id: 'advanced-metrics' },
    { time: 190, id: 'suggested-actions' },
    { time: 207, id: 'digest' }
  ];
  ...
  const handleStop = () => {
    ...
    if (audio) {
      ...
      setActiveSection(null);
      // Reset scroll cooldown when stopping
      scrollCooldown.current = {};
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f1117]/80 text-[#f3f4f6] font-sans relative">
      ...
      <header className="sticky top-0 z-[100] w-full bg-[#1c1e26]/50 backdrop-blur-2xl border-b border-white/10 shadow-[0_8px_32px_0_rgba(31,38,135,0.15)]">
        <div className="max-w-7xl mx-auto px-6 flex items-center h-16 justify-between">
          ...
          <div className="flex items-center space-x-3 relative">
            ...
            {/* Present Button */}
            {!presentMode && selectedCompany === 'coffee' && (
              <button 
            ...
            {!presentMode && selectedCompany !== 'coffee' && (
              <button 
                disabled
                className="flex items-center space-x-2 px-4 py-2 bg-gray-500/50 text-gray-300 rounded-full cursor-not-allowed opacity-50"
              >
                <Wand2 className="w-4 h-4" />
                <span>Present (Coffee Only)</span>
              </button>
            )}
            {presentMode && (
              <>
